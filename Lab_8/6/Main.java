// 6. Создайте класс Product с переменными группа, наименование, количество и стоимость за единицу.
...
public class Main {
    ...
    public static void main (String[] args) {
        ...
        Store A = shop.filter((GROUP, PRODUCT) -> PRODUCT.getGroup().compareTo(GROUP) == 0, "Milk"); // фильтруем по группе
        A.print(); // выводим отфильтрованный

        Store B = shop.filter((AMOUNT, PRODUCT) -> PRODUCT.getAmount() <= AMOUNT, 150); // фильтруем по количеству
        B.print(); // выводим отфильтрованный

        Store C = B.filter((GROUP, PRODUCT) -> PRODUCT.getGroup().compareTo(GROUP) == 0, "Oil"); // фильтруем по группе
